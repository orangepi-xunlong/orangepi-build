...
if [[ -z $BRANCH ]]; then
	...
	[[ $KERNEL_TARGET == *next* ]] && options+=("next" "Use the latest kernel")
 	[[ $KERNEL_TARGET == *master* ]] && options+=("master" "REPO KERNEL.ORG STABLE from https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git")
...
CHOSEN_KERNEL=linux-image-${KERNELBRANCH}-${LINUXFAMILY}
CHOSEN_ROOTFS=${BSP_CLI_PACKAGE_NAME}
CHOSEN_DESKTOP=orangepi-${RELEASE}-desktop-${DESKTOP_ENVIRONMENT}
CHOSEN_KSRC=linux-source-${KERNELBRANCH}-${LINUXFAMILY}
